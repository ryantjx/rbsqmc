...
### 6.5 Progress log (2026-08-13 14:11 BST)

**What has been fixed (numerical stability).** A sequence of changes eliminated the NaN/divergence failures and made the EM run to completion:

- **`scale` fixed at 1.0** — removed as a parameter, breaking the $\Gamma_0$/`scale` scale-identifiability degeneracy.
- **`gamma_0` initialized as the regional correlation matrix** `C0` (diagonal 1), PSD-projected so it is always a valid correlation matrix.
- **`_KAPPA_MIN = 0.001` floor** — prevents the transition-term explosion (kappa → 0 collapses the transition covariance, making state movement look impossible).
- **`_KAPPA_MAX = 0.002` cap** — forces a ~1-year mean-reversion half-life so team quality persists across the long gaps between international matches (median ~43 days).
- **`between_regions = -0.1`** — the earlier `-0.4` made the correlation matrix indefinite (min eig −10.18) and, after PSD projection, near-singular (cond 86M), causing NaN. `-0.1` gives a well-conditioned matrix (min eig 0.5, cond 18.5).
- **`init_sample` dispersion** — each particle now draws from the prior instead of starting collapsed at the mean.
- **`gamma_0` prior removed** — an experiment showed it had no effect (EM does not inflate `gamma_0` once `scale` is fixed and `_KAPPA_MIN` floors the transition).

**Current status (L4 run, N=500).** The EM now runs to completion without NaN, and the transition term is stable (no longer exploding). But the model is **still not producing sane results**:

- **EM is diverging.** Log marginal likelihood declines across epochs: `[-5768, -5734, -5805, -5871, -5846, -5917, -5982, -6026, -6101, -6111]`. The fit gets worse each epoch.
- **Rankings are still noise.** Spain (+0.62) and Argentina (+0.92) are decent, but France (−0.40) is negative, England (+0.10) mediocre, Uruguay (−1.98) terrible, and weak teams (Scotland +1.11, Tunisia +0.93) are overrated.
- **Predictions are poor.** Mean log-likelihood −4.69 (worse than the earlier −3.41). Several badly off (e.g. Sweden vs Tunisia predicted 0-4, actual 5-1, ll=−18.6).

**Remaining problems (in priority order).**

1. **Loss-scaling imbalance (most likely culprit).** The per-dimension loss scaling in `_complete_log_likelihood` reweights the three terms (init/obs/transition) so they are "comparable," but the transition term still dominates the objective. EM therefore optimizes the transition fit at the expense of the observation term — which is what drives the rankings. This is the key hurdle from Section 6.4: the loss scaling and the (dead) per-parameter learning rates must be considered together. Fixing this is the most likely path to sane rankings.

2. **EM divergence.** The log marginal declines every epoch, meaning the M-step is not finding a good optimum. This is likely a consequence of the loss-scaling imbalance (the objective being optimized is not the right one), possibly compounded by the MCEM noise from a finite number of smoothed trajectories.

3. **Bootstrap proposal (secondary).** The filter uses the transition as the proposal, which is a correct and standard design choice. Its known limitation is weight degeneracy when the observation is very informative, but the ESS is healthy (~242/500), so this is **not** the current problem. It is listed only as a possible future improvement, not a cause of the current failures.

**Note on the bootstrap proposal.** Using a bootstrap proposal is correct — the importance weights are valid and the filter is unbiased. It is not an error; it is a standard, robust design. The current failures are not caused by it.
