...
It reuses the GD machinery from ``rbpf_ou_v2/src/train.py`` (parameter set:
estimates ``gamma_0``, ``B``, ``alpha``, ``beta``, ``kappa``; ``mean_0``
fixed). Runtime configuration (N, steps, dates, teams, hardware) is read from
``smoothing_gpu_config.json``.

On Colab the repo is cloned/updated and the ``rbpf_ou_v2`` package is made
importable *after* the bootstrap, which is why ``rbpf_ou_v2`` imports happen
...
Writes (into ``rbpf_ou_v2/outputs_gpu``):
  - gd_params_init.json
...
# Repo root is the parent of the rbpf_ou_v2 package directory (e.g. .../rbsqmc).
...
TEST_DIR = os.path.join(REPO_DIR, "rbpf_ou_v2")
...
_DEFAULT_CONFIG = {
    ...
    "max_goals": 8,
    "output_dir": "rbpf_ou_v2/outputs_gpu",
    "hardware": "gpu",
    "gpu_type": "T4",
    "tpu_type": "v5e1",
    "colab_timeout": 3600,
...
def bootstrap():
    ...
    # Let rbpf_ou_v2/src modules (which default to cpu) use the requested
...
# ---------------------------------------------------------------------------
# GD core (imports rbpf_ou_v2 lazily so it works on a freshly-cloned Colab VM)
# ---------------------------------------------------------------------------
def run_gd(n_particles: int, start_date: str):
    ...
    from rbpf_ou_v2.src.data import get_results, ACTIVE_TEAMS, WORLDCUP_2026_TEAMS
    from rbpf_ou_v2.src.helpers import default_init_params, params_to_dict
    from rbpf_ou_v2.src.train import MAX_GOALS, run_gd as run_gd_trainer
    ...
    try:
        from rbpf_ou_v2.src.graphic import plot_em_convergence
        plot_em_convergence(
...
def main():
    ...
    log("  Reuses rbpf_ou_v2/src/train.py direct GD (-log Z).")
    log("=" * 60)
