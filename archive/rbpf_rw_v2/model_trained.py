"""Run the filter with trained GD parameters and produce diagnostic graphics.

Loads a trained ``EMParams`` JSON (by default the GPU GD output), runs the
forward filter over the full dataset, and writes the plots defined in
``rbpf_rw_v2/src/graphic.py``.

Usage:
    python model_trained.py [--params-path PATH] [--output-dir DIR]

Defaults:
    --params-path  rbpf_rw_v2/outputs_gpu/gd_params_final.json
    --output-dir   rbpf_rw_v2/outputs/smoothed
"""
...
from archive.rbpf_rw_v2.src.data import get_results, WORLDCUP_2026_TEAMS, ACTIVE_TEAMS
from archive.rbpf_rw_v2.src.helpers import load_params, generate_augmented_data
from archive.rbpf_rw_v2.src.model import run_filter, compute_gamma_trajectory
from archive.rbpf_rw_v2.src.graphic import (
    plot_top_strengths,
...
# count as the GD run (otherwise the graphics are generated on a different
...
# Use the SAME team set as the GD run (config "teams": ACTIVE_TEAMS or
...
def main():
    ...
    parser.add_argument(
        "--params-path",
        type=str,
        default="rbpf_rw_v2/outputs_gpu/gd_params_final.json",
        help="Path to trained EMParams JSON.",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="rbpf_rw_v2/outputs/smoothed",
