...
from archive.rbpf_rw_v2.src.bivariate_poisson import loglik
from archive.rbpf_rw_v2.src.data import get_results, FootballResults
from archive.rbpf_rw_v2.src.helpers import default_init_params, kron_sample_psd
from archive.rbpf_rw_v2.src.utils import RBPFState, RBPFFootballResults, EMParams
...
def _sample_psd_gaussian(
    ...
    covariance is preserved, which is what the direct-GD trainer needs to learn
    ``gamma_0``/``gamma_Q``/``B``.
    """
...
def propagate_sample(
    ...
    # Differentiable PSD-aware sampler: robust to the rare dt==0 re-observation
    # case where Sigma_EE is singular (a team's posterior variance was zeroed).
...
def main():
    from archive.rbpf_rw_v2.src.data import WORLDCUP_2026_TEAMS, ACTIVE_TEAMS
    ...
    import numpy as np
    from archive.rbpf_rw_v2.src.graphic import plot_all
