...
where $\Gamma_{t \mid t+1} = \Gamma_t - \Gamma_t \phi_{t+1}^\top (\Gamma_{t+1}^{-})^{-1} \phi_{t+1} \Gamma_t$ is the Schur complement form for the backward conditional covariance in the feature space.
