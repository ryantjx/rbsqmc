# RBPF v2: Dual-Smoother Implementation Specification

## Implementation groups

| Group | Deliverable | Core rule |
|---|---|---|
| A | Existing data/model/filter boundary | Reuse `rbpf`; do not reimplement the filter |
| B | Shared RB and MCEM core | One Cuthbert-free mathematical implementation |
| C | `smoothing.py` | Cuthbert-integrated RB backward sampler |
| D | `smoothing_noncuthbert.py` | Direct JAX reverse-scan sampler |
...
```bash
uv run bash rbpf_v2/run_smoothing_colab.sh
```
...
./rbpf_v2/run_smoothing_colab.sh
```
...
## A. Existing data/model/filter boundary

Everything through the forward filter follows `rbpf/src/smoothing.py` and its
existing imports:
...
from rbpf.src.utils import EMParams, RBPFFootballResults
```

Preserve the existing:

- date grouping, padding, masks, and team IDs;
- bivariate-Poisson likelihood;
- covariance recursion and RB propagation;
- Cuthbert particle filter and systematic resampling;
- parameter initialization and transforms;
- `FilterStates` and `RBPFFootballResults` contracts.

Do not create another forward filter in `rbpf_v2`.

### A.1 Filter representation
...
## B. Shared RB and MCEM core

Create `rbpf_v2/src/smoothing_common.py`. It must not import Cuthbert.

Both backends use this module for Gaussian calculations, backward selection,
conditional draws, result types, complete-data densities, diagnostics, and the
MCEM driver.
...
Both backend modules expose a wrapper with the same arguments and result
schema.
...
## C. Cuthbert backend: `smoothing.py`
...
call the common batched backward step, index the compact particle tree using
...
`cuthbert.smoother(..., parallel=False)`, converts to common `SmoothedStates`,
and immediately drops Cuthbert-only state.

## D. Direct backend: `smoothing_noncuthbert.py`

This module must not import Cuthbert at module import time. Because the reused
...
Develop `rbpf_v2/run_smoothing_colab.sh` as the supported orchestration layer
...
`rbpf_v2/outputs/smoothing` for the standard deployment.
...
GPU_TYPE=L4
COLAB_TIMEOUT=7200
SESSION=rbsqmc-rbpf-v2-smoothing
```
...
print jax.devices()
run python -u -m rbpf_v2.scripts.run_smoothing
```
...
```text
rbpf_v2/outputs/smoothing/em_initial_params.json
```
...
```text
/content/rbsqmc/rbpf_v2/outputs/smoothing/progress.log
```
...
Download into the local `rbpf_v2/outputs/smoothing` directory. Required files
...
- `bash -n rbpf_v2/run_smoothing_colab.sh`;
...
7. every required artifact is downloaded into
   `rbpf_v2/outputs/smoothing` and is nonempty;
