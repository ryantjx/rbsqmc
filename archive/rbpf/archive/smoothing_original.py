...
def M_step(
    ...
    def _loss_and_components(carry):
        ...
        total = init_mean + trans_mean + obs_mean
        return total, (init_mean, trans_mean, obs_mean)
    ...
    def _step(carry, _):
        ...
        _, (init_mean, trans_mean, obs_mean) = _loss_and_components(params_carry)
        ...
        return (opt_state, params_carry), (loss, init_mean, trans_mean, obs_mean)
    ...
    # loss_trace: (total, init, transition, observation), each (n_gradient_steps,)
    loss_trace = jnp.stack(loss_trace, axis=1)  # (n_gradient_steps, 4)
...
def run_EM(
    ...
    # per-epoch complete-data loss components (init / transition / observation)
    loss_components = {"init": [], "transition": [], "observation": []}
    # per-gradient-step loss trace for each epoch: (n_epochs, n_gradient_steps, 4)
    ...
    for epoch in range(n_epochs):
        ...
        init_mean, trans_mean, obs_mean = _eval_loss_components(
            smoothed_states, model_inputs_rbpf, params, n_smoother_particles
        )
    ...
    loss_traces = jnp.stack(loss_traces, axis=0)  # (n_epochs, n_gradient_steps, 4)
    return (
        best_params,
        params_track,
