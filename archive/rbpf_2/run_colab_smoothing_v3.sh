...
# Run smoothing_v3.py on Colab GPU (V3: fixed μ₀=0, fixed Γ₀, fixed B=I₂).
# Estimates κ, α, β.
#
# Usage:  ./run_colab_smoothing_v3.sh

REMOTE_OUTPUTS="/content/rbsqmc/rbpf_2/outputs_gpu_v3"
LOCAL_OUTPUTS="./rbpf_2/outputs_gpu_v3"
SCRIPT="run_smoothing_gpu_v3.py"
SESSION="${SESSION:-rbsqmc-v3}"
...
echo "============================================================"
echo "  COLAB SMOOTHING V3 GPU — PRODUCTION RUN"
echo "  Fixed μ₀=0, fixed Γ₀ (small variance), fixed B=I₂."
echo "  Estimates κ, α, β."
...
with open('$f') as fh:
    p = json.load(fh)
print(f\"  κ = {p['init_kappa']} [ESTIMATED]\")
print(f\"  α = {p['init_alpha']} [ESTIMATED]\")
print(f\"  β = {p['init_beta']} [ESTIMATED]\")
print(f\"  friendly_scale = {p['init_friendly_scale']}\")
print(f\"  B = {p['init_B']} [FIXED]\")
print(f\"  Γ_0 shape = {len(p['init_gamma'])}x{len(p['init_gamma'][0])} [FIXED]\")
print(f\"  μ_0 shape = {len(p['init_mean'])}x{len(p['init_mean'][0])} [FIXED at 0]\")
"
        echo ""
    fi
done
...
# --- Step 5: Run model_trained_v3.py locally with the trained parameters ---
echo "[5/5] Running model_trained_v3.py with trained parameters..."
cd rbpf_2
python3 -u model_trained_v3.py --params-path ./outputs_gpu_v3/em_params_final.json --output-dir ./outputs_gpu_v3/trained || \
    echo "  WARNING: model_trained_v3.py failed"
cd ..
