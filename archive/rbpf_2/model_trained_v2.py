"""Run the RBPF filter using trained (EM-estimated) parameters from smoothing_v2.py.

Loads the smoothed parameters produced by `smoothing_v2.py` (the V2 EM algorithm
with fixed κ=1.0, B=I₂, and fixed Γ₀), runs the forward filter on the full dataset,
and generates diagnostic plots and saved results (parquet).

Usage:
    python model_trained_v2.py [--params-path ./outputs_gpu_v2/em_params_final.json]
"""
...
from archive.rbpf_2.graphic import generate_all_plots, save_results, weighted_mean
from archive.rbpf_2.model import run_filter, compute_gamma_trajectory, MAX_GOALS
from archive.rbpf_2.smoothing_v2 import load_params, EMParams
...
def find_latest_params(output_dir: str) -> str:
    ...
    if not epoch_files:
        raise FileNotFoundError(
            ...
            "Run smoothing_v2.py first to produce trained parameters."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Run RBPF filter with trained EM parameters (V2: fixed κ=1.0, B=I₂)."
    )
    parser.add_argument(
        "--params-path", type=str, default=None,
        help="Path to EM params JSON. If omitted, auto-detects the latest in ./outputs_gpu_v2.",
    )
    parser.add_argument(
        "--output-dir", type=str, default="./outputs_gpu_v2/trained",
        help="Directory to save filter results and plots. Default: ./outputs_gpu_v2/trained",
    )
    parser.add_argument(
    ...
    params_path = args.params_path or find_latest_params("./outputs_gpu_v2")
    print(f"\nLoading trained parameters from: {params_path}")
    params: EMParams = load_params(params_path)

    print(f"  κ (kappa)        = {params.init_kappa:.6f} [FIXED]")
    print(f"  α (alpha)        = {params.init_alpha:.6f}")
    print(f"  β (beta)         = {params.init_beta:.6f}")
    print(f"  friendly_scale   = {params.init_friendly_scale:.6f}")
    print(f"  μ_0 (init_mean)  shape = {params.init_mean.shape}")
    print(f"  Γ_0 (init_gamma) shape = {params.init_gamma.shape} [FIXED]")
    ...
    # --- 4. Report results ---
    print(f"\n{'='*60}")
    print("FILTER COMPLETE (V2 trained parameters)")
    print(f"{'='*60}")
