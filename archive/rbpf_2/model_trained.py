"""Run the RBPF filter using trained (EM-estimated) parameters.

Loads the smoothed parameters produced by `smoothing.py` (the EM algorithm),
runs the forward filter on the full dataset, and generates diagnostic plots
and saved results (parquet) using the trained parameter set.

Usage:
    python model_trained.py [--params-path ./outputs/em_params_final.json]
"""
...
from archive.rbpf_2.smoothing import load_params, EMParams
...
def find_latest_params(output_dir: str) -> str:
    ...
    if not epoch_files:
        raise FileNotFoundError(
            ...
            "Run smoothing.py first to produce trained parameters."
        )
    return epoch_files[-1]


def main():
    parser = argparse.ArgumentParser(description="Run RBPF filter with trained EM parameters.")
    parser.add_argument(
        "--params-path", type=str, default=None,
        help="Path to EM params JSON. If omitted, auto-detects the latest in ./outputs.",
    )
    parser.add_argument(
        "--output-dir", type=str, default="./outputs/trained",
        help="Directory to save filter results and plots. Default: ./outputs/trained",
    )
    parser.add_argument(
    ...
    print(f"  κ (kappa)        = {params.init_kappa:.6f}")
    ...
    print(f"  Γ_0 (init_gamma) shape = {params.init_gamma.shape}")
    print(f"  B   (init_B)     shape = {params.init_B.shape}")
    ...
    # --- 4. Report results ---
    print(f"\n{'='*60}")
    print("FILTER COMPLETE (trained parameters)")
    print(f"{'='*60}")
