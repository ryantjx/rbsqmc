"""Bootstrap script for running rbpf_2/smoothing_v3.py on a Google Colab GPU.

V3: Fixed μ₀=0, fixed Γ₀ (small variance), fixed B=I₂. Estimates κ, α, β.

Usage:
    colab run --gpu T4 --keep run_smoothing_gpu_v3.py [GPU_N]
"""
...
OUTPUT_DIR = os.path.join(RBPF_DIR, "outputs_gpu_v3")
...
PATCH_TARGETS = ["smoothing_v3.py", "model.py", "data.py"]
...
def main():
    log("=" * 60)
    log("SMOOTHING V3 GPU RUNNER — STARTING")
    log("  Fixed μ₀=0, fixed Γ₀ (small variance), fixed B=I₂. Estimates κ, α, β.")
    log("=" * 60)
    ...
    if len(sys.argv) > 1:
        ...
        log("Using N from smoothing_v3.py (no override)")
    ...
    os.makedirs("outputs_gpu_v3", exist_ok=True)
    log("Created outputs_gpu_v3/ directory")

    log("Running smoothing_v3.py (unbuffered, streaming output)...")
    log("=" * 60)
    run_streaming([sys.executable, "-u", "smoothing_v3.py"])

    log("=" * 60)
    log("smoothing_v3.py COMPLETED")
    log("=" * 60)
    ...
    if os.path.exists(final_path):
        with open(final_path, "r") as f:
            params = json.load(f)
        log("Final EM parameters (V3):")
        log(f"  κ (kappa)     = {params.get('init_kappa', 'N/A')} [ESTIMATED]")
        log(f"  α (alpha)     = {params.get('init_alpha', 'N/A')} [ESTIMATED]")
        log(f"  β (beta)      = {params.get('init_beta', 'N/A')} [ESTIMATED]")
        log(f"  friendly_scale = {params.get('init_friendly_scale', 'N/A')}")
        log(f"  B = {params.get('init_B', 'N/A')} [FIXED]")
        log(f"  Γ_0 shape     = {len(params.get('init_gamma', []))}x{len(params.get('init_gamma', [[]])[0]) if params.get('init_gamma') else 'N/A'} [FIXED]")
        log(f"  μ_0 shape     = {len(params.get('init_mean', []))}x{len(params.get('init_mean', [[]])[0]) if params.get('init_mean') else 'N/A'} [FIXED at 0]")
    else:
