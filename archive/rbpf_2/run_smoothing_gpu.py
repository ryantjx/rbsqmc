"""Bootstrap script for running rbpf_2/smoothing.py on a Google Colab GPU.
...
Then download the outputs:
    ...
    colab download -s <session> /content/rbsqmc/rbpf_2/outputs ./outputs
...
def main():
    ...
    # --- Step 3c: Patch N if specified (otherwise use smoothing.py's default) ---
    # With WORLDCUP_2026_TEAMS filter (48 teams), the Kronecker product is (96×96)
    # instead of (524×524), so much higher N is feasible on a T4.
    # Pass via sys.argv: colab run --gpu T4 --keep run_smoothing_gpu.py 5000
    if len(sys.argv) > 1:
        GPU_N = sys.argv[1]
        ...
        log("Using N from smoothing.py (no override)")
