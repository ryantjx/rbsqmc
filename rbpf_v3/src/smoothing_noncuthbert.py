"""Standalone Cuthbert-free Rao--Blackwellized FFBS and MCEM backend."""
...
def _systematic_indices(key: jax.Array, log_weights: jax.Array, n: int) -> jax.Array:
    weights = jax.nn.softmax(log_weights)
    offset = jax.random.uniform(key, (), dtype=weights.dtype) / n
    positions = offset + jnp.arange(n, dtype=weights.dtype) / n
    return jnp.searchsorted(jnp.cumsum(weights).at[-1].set(1.0), positions)
...
@partial(
    jax.jit,
    static_argnames=("n_smoother_particles", "return_backward_probabilities"),
)
def _rb_backward_simulation_jit(
    key: jax.Array,
    means: jax.Array,
    ...
    *,
    ...
    return_backward_probabilities: bool,
) -> SmoothedStates:
    n_times, n_components = means.shape[:2]
    terminal_index_key, terminal_draw_key, scan_key = jax.random.split(key, 3)
    terminal_indices = _systematic_indices(
        terminal_index_key, log_weights[-1], n_smoother_particles
    )
    b_sqrt = jnp.linalg.cholesky(B)
    terminal = sample_kron_psd_batched(
        terminal_draw_key,
        means[-1, terminal_indices],
        psd_sqrt(gamma_filtered[-1]),
        b_sqrt,
    )
    time_keys = jax.random.split(scan_key, n_times - 1)

    def body(x_next, inputs):
        means_t, weights_t, gamma_t, pred_t, phi_t, step_key = inputs
        previous, indices, probabilities = batched_backward_step(
            step_key,
            means_t,
            weights_t,
            x_next,
        ...
        maximum = jnp.max(probabilities)
        unique = _unique_count(indices, n_components)
        return previous, (previous, indices, ess, entropy, maximum, unique, probabilities)
    ...
    previous, indices, ess, entropy, maximum, unique, probabilities = outputs
    paths = jnp.concatenate([previous, terminal[None]], axis=0)
    all_indices = jnp.concatenate([indices, terminal_indices[None]], axis=0)
    terminal_probabilities = jnp.broadcast_to(
        jax.nn.softmax(log_weights[-1]), (n_smoother_particles, n_components)
    )
    terminal_ess = 1.0 / jnp.sum(terminal_probabilities[0] ** 2)
    terminal_entropy = -jnp.sum(
        jnp.where(
            terminal_probabilities[0] > 0,
            terminal_probabilities[0] * jnp.log(terminal_probabilities[0]),
            0,
        )
    )
    diagnostics = BackwardDiagnostics(
        ess_by_time=jnp.concatenate([ess, terminal_ess[None]]),
        entropy_by_time=jnp.concatenate([entropy, terminal_entropy[None]]),
        max_probability_by_time=jnp.concatenate(
            [maximum, jnp.max(terminal_probabilities)[None]]
        ),
        unique_indices_by_time=jnp.concatenate(
            [unique, _unique_count(terminal_indices, n_components)[None]]
        ),
        probabilities=(
    ...
    return SmoothedStates(paths, all_indices, diagnostics)
...
def rb_backward_simulation(
    ...
    return _rb_backward_simulation_jit(
        key,
        jax.lax.stop_gradient(filter_states.particles.x),
...
def run_mcem(
    ...
    *,
    ...
    """Run direct-backend MCEM with fixed-path non-worsening acceptance."""
    ...
    rng = key
    last = None
    progress("backend=noncuthbert EM loop starting")
    ...
    progress("backend=noncuthbert MCEM complete")
    return {
        "backend": "noncuthbert",
