...
\section{Empirical evaluation --- GPU vs CPU}

% mention that GPU jit compile is excluded
% mention that float64 is slower on consumer GPUs()

% PLOTS
% per-time step breakdown - time per step for SQMC on GPU vs CPU with interquartile range

% log-likelihood vs particle count - SQMC requires less particles to achieve the same log-likelihood as SMC.
% particles to target error - SQMC requires less particles to achieve a lower RMSE than SMC (d = 5, target RMSE = X)
% throughput to time - SQMC generally takes longer than SMC for same number of particles (should be a point plot with interquartile range)
...
% Experimental design
% Describe:
% - random-walk/Gaussian-observation model;
% - state dimension \(d=10\);
% - \(T=100\);
% - fixed float64 precision;
% - particle counts \(64,\ldots,65536\);
% - scrambled Sobol blocks for SQMC;
% - systematic resampling for SMC;
% - separate CPU/GPU subprocesses;
% - seven steady-state timing repetitions and eight accuracy repetitions;
% - cold timing including setup, transfer, compilation, and first execution.
% End-to-end runtime scaling
% Use sqmc_smc_runtime.png.
% Report:
% - mean runtime with 95% confidence intervals;
% - empirical log-log slopes from claims_evaluation.json;
% - GPU/CPU speedups separately for SMC and SQMC;
% - particle-count crossover points;
% - SQMC/SMC runtime ratios on CPU and GPU.
% This tests whether GPU acceleration benefits SMC and SQMC, and whether SQMC incurs a runtime overhead.
% The chapter should not claim that the GPU is beneficial merely because isolated operations parallelize well. The claim must follow the end-to-end speedup intervals.
