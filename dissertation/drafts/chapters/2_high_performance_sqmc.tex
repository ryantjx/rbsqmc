...
This section evaluates whether accelerator hardware improves the three operations that distinguish SQMC from a standard bootstrap particle filter: QMC point generation, Hilbert sorting of QMC-generated inputs, and complete SQMC filtering trajectories. Each experiment compares a shared JAX implementation on CPU and GPU, so that the measured differences isolate the effect of the execution backend.

\subsection{Experimental design and measurement}
\label{sec:empirical-design}

The experiments were conducted on an NVIDIA A100-SXM4 with 40\,GB of device memory and 12 logical cores of an Intel Xeon CPU at 2.20\,GHz. More information about the hardware and software is available in the Appendix.
...
Three implementations are compared for Sobol' and Halton sequence generation: JAX on CPU, JAX on GPU and SciPy on CPU. A benefit of implementing the QMC generators in JAX is that all randomness is expressed through explicit key arrays: the random key arrays can be passed explicitly to the generator functions, which is deterministic and reproducible. In contrast, SciPy draws from a stateful \texttt{numpy.random.Generator} during engine construction, so a fresh randomisation requires constructing new Python objects, and the resulting state cannot be carried through compiled code, incurring additional construction cost included in the timing for SciPy.

We consider dimensions and point counts
\begin{gather*}
    d\in\{2,5,10,30,60\}, \\
    N\in\{128,512,2048,8192,32768,131072\}.
\end{gather*}

Each timing is the median of seven executions after two warm-up runs. This is to ensure that the timing measurements are stable and not affected by any initial overheads such as JAX tracing or compilation. 

Figure~\ref{fig:comparison-qmc} reports 120 fresh-scramble timing configurations. The SciPy CPU/JAX GPU ratio measures the effect of changing both the library and the execution backend. Values above one favour JAX GPU. 

\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{comparison_qmc_08092026_0048.pdf}
    \caption{Fresh scrambling and point generation for Sobol' (top) and Halton (bottom). Left: median runtime for JAX GPU and SciPy CPU. Right: SciPy CPU/JAX GPU median-runtime ratio, with equal runtime marked at one. Colours identify dimensions and line styles identify implementations. Every timed call includes fresh scrambling; no unscrambled or fixed-scramble sampling curve is shown.}
    \label{fig:comparison-qmc}
\end{figure}

For Sobol' generation, JAX GPU is faster than SciPy CPU at the smallest tested count in every dimension. For Halton generation, the first observed JAX GPU advantage over SciPy CPU occurs at $N=2048$ in every dimension.
...
The Hilbert-sorting experiment uses the same five dimensions as the QMC generation, with seven point counts
\begin{gather*}
    N\in\{128,256,512,2048,8192,32768,131072\},
\end{gather*}
which extends the QMC generation grid by $N=256$. Input generation is excluded from sorting time. The timed sort includes standardisation, coordinate transformation, quantisation, Hilbert-index construction and the final ordering. The CPU implementation uses the same shared JAX sorter as the GPU, so the CPU/GPU ratio measures the effect of changing the execution backend within the same implementation.
...
GPU speedups are observed at higher dimensions and particle counts. At $N=128$, GPU is slower than CPU in every tested dimension for both input families. For Sobol' inputs, the first measured GPU advantage occurs at $N=2048$ for $d=2,5$, $N=512$ for $d=10,30$, and $N=256$ for $d=60$. For Halton inputs, the corresponding counts are $2048,512,512,256,256$ in increasing dimension order. Thus the benefit of parallel sorting emerges only in higher dimensions and particle counts.
...
We compare SQMC on GPU and CPU on two complementary metrics: runtime, which measures the speed of iteration, and time to reach a target error, which measures how quickly the algorithm converges to a desired accuracy.
...
We now consider how quickly each backend can achieve a target error. The target error is defined as the normalised RMSE of the filtering mean estimates given by the equation
...
Since both backends use the same generator and estimator, the filtering error is effectively a function of the particle count alone. At different $N$, the GPU and CPU errors should be the same. The difference is how long each backend takes to run that count. Measured at the largest tested particle count, as seen in Figure~\ref{fig:comparison-sqmc-budget}, for $N=2048$ the GPU reaches the accuracy of the CPU in $6.23$--$13.37\times$ less device time across the five dimensions; for example at $d=10$ both backends attain a normalised RMSE of approximately $0.298$ with $N=2048$ particles, but the CPU requires $352.6$\,ms per trajectory whereas the GPU requires $40.6$\,ms. 
...
All code is written in the public repository \href{https://github.com/ryantjx/sqmc-cuthbert}{\texttt{ryantjx/sqmc-cuthbert}} and will be integrated into \texttt{state-space-models/cuthbert} under the issue \href{https://github.com/state-space-models/cuthbert/issues/255}{Implement Sequential Quasi Monte Carlo \#255}.
