...
Three implementations are compared for Sobol' and Halton sequence generation: \texttt{jax} on CPU, \texttt{jax} on GPU and \texttt{scipy} on CPU. A benefit of implementing the QMC generators in \texttt{jax} is that all randomness is expressed through explicit key arrays: the random key arrays can be passed explicitly to the generator functions, which is deterministic and reproducible. In contrast, \texttt{scipy} draws from a stateful \texttt{numpy.random.Generator} during engine construction, so a fresh randomisation requires constructing new Python objects, and the resulting state cannot be carried through compiled code, incurring additional construction cost included in the timing for \texttt{scipy}.
...
Each timing is the median of seven executions after two warm-up runs. This is to ensure that the timing measurements are stable and not affected by any initial overheads such as \texttt{jax} tracing or compilation. 

Figure~\ref{fig:comparison-qmc} reports 120 fresh-scramble timing configurations. The \texttt{scipy} CPU/\texttt{jax} GPU ratio measures the effect of changing both the library and the execution backend. Values above one favour \texttt{jax} GPU. 

\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{results/qmc_hilbert_sqmc/comparison_qmc_08092026_0048.pdf}
    \caption{Fresh scrambling and point generation for Sobol' (top) and Halton (bottom). Left: median runtime for \texttt{jax} GPU and \texttt{scipy} CPU. Right: \texttt{scipy} CPU/\texttt{jax} GPU median-runtime ratio, with equal runtime marked at one. Colours identify dimensions and line styles identify implementations. Every timed call includes fresh scrambling; no unscrambled or fixed-scramble sampling curve is shown.}
    \label{fig:comparison-qmc}
\end{figure}

For Sobol' generation, \texttt{jax} GPU is faster than \texttt{scipy} CPU at the smallest tested count in every dimension. For Halton generation, the first observed \texttt{jax} GPU advantage over \texttt{scipy} CPU occurs at $N=2048$ in every dimension.
...
\end{gather*}
which extends the QMC generation grid by $N=256$. Input generation is excluded from sorting time. The timed sort includes standardisation, coordinate transformation, quantisation, Hilbert-index construction and the final ordering. The CPU implementation uses the same shared \texttt{jax} sorter as the GPU, so the CPU/GPU ratio measures the effect of changing the execution backend within the same implementation.

Figure~\ref{fig:comparison-hilbert} separates Sobol'-generated and Halton-generated inputs. The 140 configurations compare the same shared \texttt{jax} sorter on CPU and GPU, using identical normal-transformed arrays within each pair. \texttt{scipy} and the \texttt{particles} library are not timed sorting baselines.

\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{results/qmc_hilbert_sqmc/comparison_hilbert_08092026_0048.pdf}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{results/qmc_hilbert_sqmc/comparison_sqmc_runtime_08092026_0048.pdf}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{results/qmc_hilbert_sqmc/comparison_sqmc_budget_08092026_0048.pdf}
