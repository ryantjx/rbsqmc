...
\paragraph{Runtime budgets as a secondary comparison.}
The same measurements can also be used to choose a particle count under a runtime cap. This allows CPU and GPU to use different values of $N$, unlike the matched-work comparison above.

For a budget $B$, eligibility is determined by the measured median trajectory time $\widetilde\tau_{b,d,N}$ on backend $b$. Among eligible counts, the selection-stage error determines the choice:
\begin{equation}
    \widehat N_{b,d}(B)
...
\end{equation}
Ties are resolved by runtime and then particle count. An empty eligible set is recorded as infeasible; the procedure does not extrapolate beyond the tested grid. Validation error is evaluated after this decision and is not used to select $N$.

The fixed-budget accuracy advantage arises from the larger particle counts that the GPU can process, with the same statistical estimator on both backends. Table~\ref{tab:sqmc-budget} gives the selected counts, measured runtimes and held-out errors. No configuration on either backend meets the 10\,ms budget, so those infeasible cases are omitted from the table.
...
Figure~\ref{fig:comparison-sqmc-budget} displays the selected configurations' validation errors and conditional bootstrap intervals. Budget selection can assign different particle counts to CPU and GPU; any resulting error reduction is a consequence of that allocation, whereas the fixed-work table compares the same estimator size on both devices.
