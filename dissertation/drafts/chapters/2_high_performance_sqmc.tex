...
Figure~\ref{fig:comparison-qmc} reports 180 fresh-scramble timing configurations: two sequences, five dimensions, six point counts and three implementations. The left column shows median runtime for JAX CPU, JAX GPU and SciPy CPU. The right column gives two separate speedups, JAX CPU/JAX GPU and SciPy CPU/JAX GPU. Values above one favour JAX GPU. The JAX CPU/JAX GPU ratio measures the effect of changing the execution backend within the shared implementation. The SciPy CPU/JAX GPU ratio additionally includes a change of library and execution model.
...
\begin{figure}[t]
    ...
    \caption{JAX CPU/JAX GPU median-runtime ratios for shared Hilbert sorting. Left: normal-transformed scrambled Sobol' inputs; right: normal-transformed scrambled Halton inputs. Colours identify dimensions. Point generation and input transfer are excluded from sorting time; the horizontal line marks equal runtime.}
    \label{fig:comparison-hilbert}
\end{figure}

The GPU is slower at $N=128$ in every tested dimension for both input families. For Sobol' inputs, the first measured GPU advantage occurs at $N=2048$ for $d=2,5$, $N=512$ for $d=10,30$, and $N=256$ for $d=60$. For Halton inputs, the corresponding counts are $2048,512,512,256,256$ in increasing dimension order. Thus the benefit of parallel sorting emerges only after sufficient points are processed. At $N=131072$, speedups reach $79.42$--$107.93\times$ for Sobol' inputs and $78.30$--$109.14\times$ for Halton inputs.
