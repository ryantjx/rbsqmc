"""Validate downloaded model_unbiased artifacts without importing JAX.
...
def validate(directory: Path) -> None:
    ...
    for name in ("params_unbiased.json", "run_config.json", "optimization_summary.json"):
        value = json.loads((directory / name).read_text(encoding="utf-8"))
        if not _finite(value):
            raise ValueError(f"non-finite numeric value in {name}")

    params = json.loads((directory / "params_unbiased.json").read_text(encoding="utf-8"))
    for key in ("kappa", "alpha", "beta"):
        if key not in params:
            raise ValueError(f"missing key in params_unbiased.json: {key}")
        if not math.isfinite(float(params[key])):
            raise ValueError(f"non-finite value for {key}: {params[key]}")

    summary = json.loads((directory / "optimization_summary.json").read_text(encoding="utf-8"))
    for key in ("baseline_logZ", "best_logZ", "final_filter_logZ"):
        if key not in summary:
            raise ValueError(f"missing key in optimization_summary.json: {key}")
        if not math.isfinite(float(summary[key])):
            raise ValueError(f"non-finite value for {key}: {summary[key]}")

    print(f"Validation passed: {len(REQUIRED)} artifacts OK in {directory}")


def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description="Validate model_unbiased outputs")
