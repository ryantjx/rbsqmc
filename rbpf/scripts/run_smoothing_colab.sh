#!/usr/bin/env bash
# Orchestrator: launch the Adam smoothing EM pipeline on a Colab GPU.
#
...
#   bash rbpf/scripts/run_smoothing_colab.sh --dry-run
#
# Environment overrides:
#   GPU_TYPE       e.g. A100, T4 (default: from config)
#   COLAB_TIMEOUT  seconds (default: from config)
#   SESSION        colab session name (default: rbsqmc-rbpf-adam-smoothing)

set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${HERE}/../.." && pwd)"
CONFIG="${HERE}/config/smoothing_gpu_config.json"
