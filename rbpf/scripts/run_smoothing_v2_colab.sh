#!/usr/bin/env bash
# Orchestrator: launch the v2 (Adam) smoothing EM pipeline on a Colab GPU.
#
# This script runs LOCALLY. It:
#   1. Validates the config and bootstrap script.
#   2. Launches run_smoothing_v2_gpu.py on a Colab GPU session via `colab run`.
...
#   bash rbpf/scripts/run_smoothing_v2_colab.sh --dry-run
#
# Environment overrides:
#   GPU_TYPE       e.g. A100, T4 (default: from config)
#   COLAB_TIMEOUT  seconds (default: from config)
#   SESSION        colab session name (default: rbsqmc-rbpf-adam-smoothing-v2)

set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${HERE}/../.." && pwd)"
CONFIG="${HERE}/config/smoothing_v2_gpu_config.json"
BOOTSTRAP="${HERE}/run_smoothing_v2_gpu.py"
...
validate_local_outputs() {
    # Lightweight check: the strict validate_outputs.py requires
    # filter/initial_correlation_matrix.png, which smoothing_v2.py does not
    # produce (its plot_all call passes no params=). So validate against the
    # actual artifact list instead.
    local missing=0 file
    for file in "${required[@]}"; do
        if [[ ! -s "${LOCAL_OUTPUTS}/${file}" ]]; then
            progress ERR "missing or empty artifact: ${file}"
            missing=1
        fi
    done
    ...
    progress OUT "validation passed: ${#required[@]} artifacts OK in ${LOCAL_OUTPUTS}"
}
...
main() {
    ...
    SESSION="${SESSION:-rbsqmc-rbpf-${m_step}-smoothing-v2}"
    ...
    progress OUT "launching RBPF v2 ${m_step} smoothing on Colab GPU=${gpu_type}"
    SESSION_LAUNCHED=1
    ...
    validate_local_outputs
    progress OUT "RBPF v2 ${m_step} Colab acceptance completed successfully"
}
