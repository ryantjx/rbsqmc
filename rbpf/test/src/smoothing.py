...
def _smoother_rts_single(
    ...
    Adapted from the OU reference to the random-walk model: the transition is
    ``X_{t+1} = X_t + eps`` with ``eps ~ N(0, dt * Q)``, so the prediction mean
    is the particle state itself (no mean reversion) and the RTS gain is
    ``J_t = Sigma_{t|t} Sigma_{t+1|t}^{-1}``.
    """
    ...
    def backward_sampling(carry, xs):
        ...
        # branch (which inverts Sigma_RTS) is only executed when dt > 0.
...
def M_step(
    ...
    print(f"      M-step done: loss {float(loss_best):.4f} -> best at step {best_step}, "
          f"alpha={float(final.alpha):.5f} beta={float(final.beta):.5f}")
