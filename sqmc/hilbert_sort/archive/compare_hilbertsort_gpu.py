...
def setup_jax_config(backend: str = "cpu"):
    """Configure JAX for 64-bit or 32-bit based on backend."""
    try:
    ...
    try:
        tpu_available = jax.device_count('tpu') > 0
    except:
        tpu_available = False
    
    # Enable 64-bit for TPU (full support)
    if backend == "tpu" and tpu_available:
        jax.config.update("jax_enable_x64", True)
        print("  Detected TPU - using 64-bit (float64/int64)")
        return True
    
    # Enable 64-bit for CUDA GPU, disable for Metal/CPU
    elif backend == "gpu" and gpu_available:
        # Check if it's Metal (macOS) or CUDA
        from jax._src import xla_bridge
        platform = xla_bridge.get_backend().platform
        if platform == "gpu" and not hasattr(jax.lib, 'xla_client'):
            # Likely Metal - use 32-bit
            jax.config.update("jax_enable_x64", False)
            print("  Detected Metal GPU - using 32-bit (float32/int32)")
            return False
        else:
            # Likely CUDA - use 64-bit
            jax.config.update("jax_enable_x64", True)
            print("  Detected CUDA GPU - using 64-bit (float64/int64)")
            return True
    else:
        # CPU or Metal - use 32-bit for compatibility
        jax.config.update("jax_enable_x64", False)
        print(f"  Using {backend.upper()} - using 32-bit (float32/int32)")
        return False
...
def run_benchmark(backend: str = "cpu"):
    ...
    jax_hilbert_sort = jax.jit(hilbert_sort.hilbert_sort, backend=backend)
    ...
    for i, n in enumerate(N_VALUES):
        ...
        for j, d in enumerate(DIMENSIONS):
            ...
            dtype = np.float64 if USE_64BIT else np.float32
            x = np.random.randn(n, d).astype(dtype)
            
            # --- Numba Benchmark ---
            # Warm-up run
            _ = hilbert_particles.hilbert_sort(x)
...
def main():
    ...
    if JAX_BACKEND == "tpu":
        try:
            if jax.device_count('tpu') == 0:
                print("\n⚠ TPU requested but not available. Falling back to CPU.")
                JAX_BACKEND = "cpu"
        except:
            print("\n⚠ TPU requested but not available. Falling back to CPU.")
            JAX_BACKEND = "cpu"
    ...
    # Setup JAX config based on backend
    global USE_64BIT
    USE_64BIT = setup_jax_config(JAX_BACKEND)
    
    # Set MAX_BITS based on 64-bit support
    if USE_64BIT:
        hilbert_sort.MAX_BITS = 62  # Full precision for CUDA
    else:
        hilbert_sort.MAX_BITS = 30  # Safe for Metal/CPU
    print(f"\nParameters:")
    print(f"  JAX Backend: {JAX_BACKEND.upper()}")
    print(f"  64-bit Support: {USE_64BIT}")
