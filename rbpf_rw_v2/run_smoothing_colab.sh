...
# Run rbpf_rw_v2/smoothing_gpu.py on a Colab GPU.
#
# Reuses the direct-gradient-descent machinery from rbpf_rw_v2/src/train.py
# (parameter set: estimates gamma_0, gamma_Q, B, alpha, beta; mean_0 fixed) but
# runs with a GPU-oriented configuration.
#
...
# config output_dir is repo-root-relative (e.g. "rbpf_rw_v2/outputs_gpu").
...
SESSION="${SESSION:-rbsqmc-test-rw-v2}"
...
echo "============================================================"
echo "  COLAB SMOOTHING GPU — DIRECT-GD RANDOM-WALK TEST RUN"
echo "  Reuses rbpf_rw_v2/src/train.py direct GD (-log Z)."
echo "  ${ACCEL_LABEL}, N=${GPU_N}, start_date=$(read_config start_date), n_steps=$(read_config n_steps), teams=$(read_config teams)"
...
# The Colab runner writes gd_params_final.json into the GPU output dir. Use it
# as the trained params unless the caller overrides PARAMS_PATH.
TRAINED_PARAMS="${PARAMS_PATH:-${LOCAL_OUTPUTS}/gd_params_final.json}"
...
echo "  GD params:      ${LOCAL_OUTPUTS}/gd_params_{init,final}.json"
echo "  Log marginal:   ${LOCAL_OUTPUTS}/gd_log_marginal_history.json"
echo ""
echo "  GD output files:"
for f in "${OUTPUT_FILES[@]}"; do
